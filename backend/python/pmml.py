...
gwa_pipeline = PMMLPipeline([("regressor", gwa_model)])
gwa_pipeline.fit(X_train_gwa, y_train_gwa)
y_pred_gwa = gwa_pipeline.predict(X_test_gwa)
...
success_rate_pipeline = PMMLPipeline([("regressor", success_rate_model)])
success_rate_pipeline.fit(X_train_sr, y_train_sr)
y_pred_sr = success_rate_pipeline.predict(X_test_sr)
...
# Save Models as PMML
sklearn2pmml(gwa_pipeline, "gwa_model.pmml", with_repr=True)
sklearn2pmml(success_rate_pipeline, "success_rate_model.pmml", with_repr=True)

print("\nModels saved as PMML: gwa_model.pmml, success_rate_model.pmml")
